<?php
	...
	if(isset($_REQUEST['excel']) && isset($_REQUEST['pass']) && isset($_REQUEST['moderator_id']) && $_REQUEST['moderator_id']==$_SESSION['moderator_id'])
	{
		$excel_name='1';
			...
			if($excel_name!="1")
			{
				if ( $xlsx = SimpleXLSX::parse('../../excel_files/uploaded/'.$excel_name) ) 
				{
					...
					foreach ( $xlsx->rows() as $r => $row ) {
						...
						$stmt = $conn->prepare("select * from nr_student where nr_stud_id=:student_id and nr_prog_id in (select nr_prog_id from nr_course where nr_course_code=:course_code and nr_course_status='Active')");
