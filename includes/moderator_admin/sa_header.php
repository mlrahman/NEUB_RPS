	...
	<head>
		...
		<script>
			...
			function check_login()
			{
				...
				xmlhttp.open("GET", "../includes/moderator_admin/check_login.php?moderator_id="+<?php echo $_SESSION['moderator_id']; ?>+"&login_check=yes", true);
				xmlhttp.send();
			}
	...
	<body class="w3-black">
				...
				<div class="w3-container w3-row w3-round-bottom-large w3-padding w3-border w3-border-teal" style="height:100%;">
					<form style="margin: 0 auto;" class="w3-container w3-margin-bottom" action="sa_index.php" method="post">
						...
						<div class="w3-section w3-border w3-round-large w3-padding w3-text-black w3-justify">
							...
							<input class="w3-input w3-border w3-margin-bottom w3-round-large" type="number" placeholder="Enter your OTP" autocomplete="off" name="moderator_otp" required>
							<?php 
								//spam Check 
								$aaa=rand(1,20);
								$bbb=rand(1,20);
		...
		<?php
			...
			if(isset($_REQUEST['two_check']))
			{
				...
				$moderator_otp=trim($_REQUEST['moderator_otp']);
				...
				if(count($result)==0)
				{
					$_SESSION['error']='Sorry! you have entered an invalid OTP ('.(5-($_SESSION['otp_count3']+1)).')';
					$_SESSION['otp_count3']=$_SESSION['otp_count3']+1;
					if($_SESSION['otp_count3']>=5)
					{
				...
				else
				{
					if($result[0][5]=='Active')
					{
						...
						$_SESSION['moderator_two_factor_check']='Y';
			...
			{
				$moderator_id=$_SESSION['moderator_id'];
				$sa_name=$_SESSION['moderator_name'];
			...
			if($_SESSION['moderator_two_factor_status']==1) //two factor enabled
			{
				if($_SESSION['moderator_two_factor_check']=='N') //two factor not passed yet
				{
					echo '
					<script>
