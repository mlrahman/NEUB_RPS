	...
	if(isset($_REQUEST['student_id']) && isset($_REQUEST['course_id']) && isset($_REQUEST['pass']) && isset($_REQUEST['admin_id']) && $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		try
		{
			...
			if($student_email!='' && $student_status=='Active')
			{
				...
				$stmt = $conn->prepare("select nr_course_code,nr_course_title,nr_course_credit from nr_course where nr_course_id=(select nr_course_id from nr_student_waived_credit where nr_stwacr_id=:course_id limit 1) ");
				...
				$data='<table border="2">
							...
							<tr>
								...
								<td style="padding:1px;">'.number_format($course_credit).'</td>
							</tr>
						</table>';
				...
				$msg="Dear ".$f_name.", One waived course is removed from your student ID: ".$student_id.". Please check the mentioned details in the table. You can check it from ".$title." student panel. ".$data." <p>&nbsp;</p>For any query you can contact at: <a href='mailto:".$contact_email."' target='_blank'>".$contact_email."</a>";
				$message = '<html><body>';
				$message .= '<h1>Waived Course Removed from - '.$title.'</h1><p>  </p>';
				$message .= '<p><b>Message Details:</b></p>';
				$message .= '<p>'.$msg.'</p></body></html>';
				
				
				sent_mail($student_email,$title.' - Waived Course Removed Notification',$message,$title,$contact_email);
			...
			$stmt = $conn->prepare("delete from nr_student_waived_credit where nr_stwacr_id=:course_id and nr_stud_id=:student_id ");
			$stmt->bindParam(':course_id', $course_id);
			$stmt->bindParam(':student_id', $student_id);
			$stmt->execute();
