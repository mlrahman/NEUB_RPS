...
?>		
		<p class="w3-margin-0 w3-padding-0 w3-medium"><i class="fa fa-print w3-hover-text-teal w3-text-indigo w3-cursor" onclick="print_course_offers_delete_history()"> Print</i></p>		
		...
		<table style="width:100%;margin:5px 0px 15px 0px;" class="w3-border w3-round w3-border-black w3-topbar w3-bottombar">
			...
			<?php
				$stmt = $conn->prepare("select * from nr_delete_history a,nr_admin b where a.nr_admin_id=b.nr_admin_id and a.nr_deleteh_type='Course Offer List' order by a.nr_deleteh_date desc,a.nr_deleteh_time desc ");
				$stmt->execute();
