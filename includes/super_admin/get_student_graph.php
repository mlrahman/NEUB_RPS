<?php
	...
	if(isset($_REQUEST['program_id']) && isset($_REQUEST['student_graph_to']) && isset($_REQUEST['student_graph_from']) && isset($_REQUEST['dept_id']) && isset($_REQUEST['admin_id']) && $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		...
		function fetch_student_graph_data_graduates($semester,$year)
		{
			...
			if($program_id==-1 && $dept_id==-1)
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_stud_status='Active' and b.nr_studi_graduated=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_last_semester='$semester' and b.nr_studi_last_year='$year' ");
			}
			else if($program_id==-1 && $dept_id!=-1)
			{
			...
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_prog_id=:prog_id and a.nr_stud_status='Active' and b.nr_studi_graduated=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_last_semester='$semester' and b.nr_studi_last_year='$year' ");
				$stmt->bindParam(':prog_id', $program_id);
			}
			else
			{
		...
		function fetch_student_graph_data_dropouts($semester,$year)
		{
			...
			if($program_id==-1 && $dept_id==-1)
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_stud_status='Active' and b.nr_studi_dropout=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_drop_semester='$semester' and b.nr_studi_drop_year='$year' ");
			}
			else if($program_id==-1 && $dept_id!=-1)
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:dept_id) and a.nr_stud_status='Active' and b.nr_studi_dropout=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_drop_semester='$semester' and b.nr_studi_drop_year='$year' ");
				$stmt->bindParam(':dept_id', $dept_id);
			}
			else if($program_id!=-1 && $dept_id==-1)
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_prog_id=:prog_id and a.nr_stud_status='Active' and b.nr_studi_dropout=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_drop_semester='$semester' and b.nr_studi_drop_year='$year' ");
				$stmt->bindParam(':prog_id', $program_id);
			}
			else
			{
				$stmt = $conn->prepare("select count(a.nr_stud_id) from nr_student a,nr_student_info b where a.nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:dept_id) and a.nr_prog_id=:prog_id and a.nr_stud_status='Active' and b.nr_studi_dropout=1 and a.nr_stud_id=b.nr_stud_id  and b.nr_studi_drop_semester='$semester' and b.nr_studi_drop_year='$year' ");
				$stmt->bindParam(':dept_id', $dept_id);
				$stmt->bindParam(':prog_id', $program_id);
			}
			$stmt->execute();
