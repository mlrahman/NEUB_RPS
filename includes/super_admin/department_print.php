	...
	if(isset($_REQUEST['sort']) && isset($_REQUEST['filter_status2']) && isset($_REQUEST['search_text']) && isset($_REQUEST['admin_id']) &&  $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		...
		if(count($result)!=0)
		{
			...
			for($i=0;$i<$sz;$i++)
			{
				...
				$html=$html.'<tr class="'.$col.'" title="Status '.$result[$i][3].'">
			...
			echo $html=$html.'<tr><td colspan="6"><p class="w3-center w3-text-red" style="margin: 10px 0px 10px 0px;"><i class="fa fa-warning"></i> No result available</p> </td></tr>';
