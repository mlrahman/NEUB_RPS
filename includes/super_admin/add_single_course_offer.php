<?php
	...
	if(isset($_REQUEST['course_offer_prog']) && isset($_REQUEST['course_offer_type']) && isset($_REQUEST['course_offer_semester']) && isset($_REQUEST['course_offer_status']) && isset($_REQUEST['course_offer_course']) && isset($_REQUEST['pass']) && isset($_REQUEST['admin_id']) && $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		try{
			...
			$t=get_current_time();
			$d=get_current_date();
			$task='Added Offer Course Title: '.$course_title.', Course Code: '.$course_code.', Course Credit: '.number_format($course_credit,2).', Course Type: '.$course_offer_type.', Offer Semester: '.$course_offer_semester.', Offer Program: '.$prog_title.', Program Credit: '.$prog_credit.', Offer Status: '.$course_offer_status;
