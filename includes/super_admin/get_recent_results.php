<?php
	...
	if(isset($_REQUEST['program_id']) && isset($_REQUEST['recent_results_from']) && isset($_REQUEST['dept_id']) && isset($_REQUEST['admin_id']) && $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		...
		{
			$stmt = $conn->prepare("SELECT * FROM nr_result a,nr_course b where a.nr_course_id=b.nr_course_id and nr_result_status='Active' order by nr_result_id desc limit $page,5");
		}
		...
		{
			$stmt = $conn->prepare("SELECT * FROM nr_result a,nr_course b where a.nr_prog_id=:prog_id and a.nr_course_id=b.nr_course_id and nr_result_status='Active' order by nr_result_id desc limit $page,5");
