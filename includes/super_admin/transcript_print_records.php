...
<!-- Menu -->
	...
	<div class="w3-container w3-padding-0" style="margin:0px 0px 20px 0px;">
		<button onclick="get_total_search_results12(0,0)" class="w3-button w3-brown w3-round-large w3-hover-teal"><i class="fa fa-refresh"></i> Refresh</button>
	...
	<div class="w3-container" style="margin: 12px 0px 25px 0px;padding:0px;position:relative;">
		<div class="w3-container w3-topbar w3-bottombar w3-round-large w3-rightbar w3-leftbar w3-padding" style="margin:0 auto; width:50%;min-width:310px;">
			...
			<input type="text" id="search_text12" oninput="if(this.value!=''){ document.getElementById('search_clear_btn_12').style.display='inline-block'; } else { document.getElementById('search_clear_btn_12').style.display='none'; } get_search_result12()" class="w3-input w3-border-teal" style="width:89%;min-width:220px;display:inline;" placeholder="Enter Ref. No. or Printed By for Search"  autocomplete="off">
			<i class="fa fa-close w3-text-red w3-hover-text-teal w3-cursor w3-large" style="display:none;" id="search_clear_btn_12" title="Clear search box" onclick="document.getElementById('search_text12').value=''; document.getElementById('search_clear_btn_12').style.display='none';get_search_result12();"></i>
		</div>
	</div>
	...
	<div id="search_window12" class="w3-container w3-topbar w3-leftbar w3-rightbar w3-bottombar w3-round-large w3-margin-bottom" style="display:none;">
		...
		<span onclick="print_transcript_print_view()" title="Print Window" class="w3-button w3-right w3-large w3-indigo w3-hover-teal w3-round" style="padding:2px 10px;margin: 15px 10px 0px 0px;"><i class="fa fa-print"></i></span>
	...
	<p class="w3-margin-0 w3-padding-0 w3-medium w3-left">Total Data: <span class="w3-text-red" id="search_data_label12"></span></p>		
	<p class="w3-margin-0 w3-padding-0 w3-medium w3-left w3-margin-left"><i class="fa fa-print w3-hover-text-teal w3-text-indigo w3-cursor" onclick="print_transcript_prints()"> Print</i></p>		
	<div class="w3-clear"></div>
...
<script>
	...
	/****************************************/
	function print_transcript_prints()
	{
		...
		window.open('../includes/super_admin/transcript_record_print.php?admin_id='+<?php echo $_SESSION["admin_id"]; ?>+'&sort='+r_sort+'&search_text='+search_text+'&program_id='+prog_id+'&dept_id='+dept_id+'&user_type='+user_type);		
	}
	
	function print_transcript_print_view() { 
		var divContents = document.getElementById("search_window12").innerHTML; 
