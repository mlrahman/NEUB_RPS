<?php
	...
	if(isset($_REQUEST['admin_id']) &&  $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		...
		$stmt = $conn->prepare("select * from nr_delete_history a,nr_admin b where a.nr_admin_id=b.nr_admin_id and a.nr_deleteh_type='Faculty' order by a.nr_deleteh_date desc,a.nr_deleteh_time desc ");
		$stmt->execute();
		$result = $stmt->fetchAll();
		
		$html = $html.'<h2 style="border-bottom: 2px solid black;width:260px;">Faculty Remove History</h2>Total Data: '.count($result).'<table style="width:695px;border: 2px solid black;">
