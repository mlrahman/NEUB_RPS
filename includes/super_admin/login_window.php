<?php
	...
	$enable=0;
	if(isset($_REQUEST['forget_password2']) && isset($_REQUEST['admin_email']))
	{
		$email=trim($_REQUEST['admin_email']);
		$stmt = $conn->prepare("select * from nr_admin where nr_admin_email=:email and (nr_admin_type='Admin' or nr_admin_type='Super Admin') ");
		...
		if(count($result)==1)
		{
			if($result[0][8]=='Active')
			{
				...
				$msg="Dear ".$sa_name.", Your Password Recovery Link is: <a href='https://".$link."' target='_blank'>".$link."</a><p>&nbsp;</p><b>Note: </b>This is an one time link so do not reload the page. For any query you can contact with <a href='mailto:".$contact_email."'>".$contact_email."</a>";
				$message = '<html><body>';
		...
		<div class="w3-container" style="margin:0px;padding:0px;">
			...
			<form style="margin-top:0px;" class="w3-container w3-margin-bottom" action="index.php" method="post">				
				<div class="w3-section w3-border w3-round-large w3-padding">
				  ...
				  <input class="w3-input w3-border w3-margin-bottom w3-round-large" type="email" placeholder="Enter Email" autocomplete="off" name="admin_email" required>
...
<div class="w3-container" style="padding:30px 0px 30px 0px;min-height:500px;">
	<div class="w3-container w3-card-4 w3-animate-zoom w3-light-gray w3-round-large" style="max-width:450px;margin: 0 auto;">
		...
		<div class="w3-container" style="margin:0px;padding:0px;">
			...
			<div class="w3-container"><br>
				<h2  style="margin-bottom:0px;font-family: 'Comic Sans MS', cursive, sans-serif;" class="w3-xlarge w3-bold w3-center" >Admin Login</h2>
			</div>
			...
			<form style="margin-top:0px;" class="w3-container w3-margin-bottom" action="login_exec.php" method="post">
				...
				<div class="w3-section w3-border w3-round-large w3-padding">
				  <label><b>Email</b></label>
				  <input class="w3-input w3-border w3-margin-bottom w3-round-large" type="email" placeholder="Enter Email" autocomplete="off" name="admin_email" value="<?php if(isset($_COOKIE["admin_email"])) { echo $_COOKIE["admin_email"]; } ?>" required>
				  <label><b>Password</b></label>
				  <input class="w3-input w3-border w3-margin-bottom w3-round-large" type="password" placeholder="Enter Password" autocomplete="off" name="admin_password" value="<?php if(isset($_COOKIE["admin_password"])) { echo $_COOKIE["admin_password"]; } ?>" required>
				...
				<div class="w3-row w3-margin-bottom">
					<div class="w3-col" style="width:50%;">
						<input class="w3-check" type="checkbox" name="re_admin" <?php if(isset($_COOKIE["admin_login"])) { ?> checked <?php } ?>> Remember me	
					</div>
