<?php
	...
	if(isset($_REQUEST['prog_id']) && isset($_REQUEST['admin_id']) && $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		try
		{
			$stmt = $conn->prepare(" select nr_course_id,nr_course_code,nr_course_title from nr_course where nr_course_status='Active' and nr_course_id in (select nr_course_id from nr_result where nr_prog_id=:prog_id) ");
