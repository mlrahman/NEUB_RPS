<?php
	...
	if(isset($_REQUEST['user_type']) && isset($_REQUEST['search_text']) && isset($_REQUEST['sort']) && isset($_REQUEST['search_results_from']) && isset($_REQUEST['admin_id']) &&  $_REQUEST['admin_id']==$_SESSION['admin_id'])
	{
		...
		else if($user_type==1) //Faculty
		{
			if($sort==1)
			{
				$order_by='a.nr_falotr_date asc,';
				$order='a.nr_falotr_time asc';
			}
			else if($sort==2)
			{
				$order_by='a.nr_falotr_date desc,';
				$order='a.nr_falotr_time desc';
			}
			else if($sort==3)
			{
				$order_by='b.nr_faculty_name';
				$order='asc';
			}
			else if($sort==4)
			{
			...
			$stmt = $conn->prepare("select distinct(b.nr_faculty_id),b.nr_faculty_name,b.nr_faculty_designation,(select count(c.nr_faculty_id) from nr_faculty_login_transaction c where c.nr_faculty_id=b.nr_faculty_id),(select d.nr_falotr_date from nr_faculty_login_transaction d where d.nr_faculty_id=b.nr_faculty_id order by d.nr_falotr_date desc,d.nr_falotr_time desc limit 1),(select e.nr_falotr_time from nr_faculty_login_transaction e where e.nr_faculty_id=b.nr_faculty_id order by e.nr_falotr_date desc,e.nr_falotr_time desc limit 1) from nr_faculty_login_transaction a,nr_faculty b where a.nr_faculty_id=b.nr_faculty_id and b.nr_faculty_name like concat('%',:search_text,'%') order by ".$order_by." ".$order." limit $page,5 ");
			$stmt->bindParam(':search_text', $search_text);
			$stmt->execute();
			$result = $stmt->fetchAll();
			if(count($result)!=0)
			{
				...
				for($i=0;$i<$sz;$i++)
				{
					echo '<tr>
							...
							<td valign="top" class="w3-padding-small w3-border">Faculty</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][3].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.get_date($result[$i][4]).' at '.$result[$i][5].'</td>
							<td valign="top" class="w3-padding-small w3-border"><a class="w3-text-blue w3-cursor w3-decoration-null w3-bold" onclick="view_result13(\'Faculty\',\''.$result[$i][0].'\')"><i class="fa fa-envelope-open-o"></i> View</a></td>
						</tr>';
				}
			}
		}
		else if($user_type==2) //moderator
		{
			if($sort==1)
			{
				$order_by='a.nr_suadlotr_date asc,';
				$order='a.nr_suadlotr_time asc';
			}
			else if($sort==2)
			{
				$order_by='a.nr_suadlotr_date desc,';
				$order='a.nr_suadlotr_time desc';
			}
			else if($sort==3)
			{
				$order_by='b.nr_admin_name';
				$order='asc';
			}
			else if($sort==4)
			{
			...
			$stmt = $conn->prepare("select distinct(b.nr_admin_id),b.nr_admin_name,b.nr_admin_designation,(select count(c.nr_admin_id) from nr_admin_login_transaction c where c.nr_admin_id=b.nr_admin_id),(select d.nr_suadlotr_date from nr_admin_login_transaction d where d.nr_admin_id=b.nr_admin_id order by d.nr_suadlotr_date desc,d.nr_suadlotr_time desc limit 1),(select e.nr_suadlotr_time from nr_admin_login_transaction e where e.nr_admin_id=b.nr_admin_id order by e.nr_suadlotr_date desc,e.nr_suadlotr_time desc limit 1) from nr_admin_login_transaction a,nr_admin b where a.nr_admin_id=b.nr_admin_id and b.nr_admin_name like concat('%',:search_text,'%') and b.nr_admin_type='Moderator' order by ".$order_by." ".$order." limit $page,5 ");
			$stmt->bindParam(':search_text', $search_text);
			$stmt->execute();
			$result = $stmt->fetchAll();
			if(count($result)!=0)
			{
				...
				for($i=0;$i<$sz;$i++)
				{
					echo '<tr>
							...
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][1].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][2].'</td>
							<td valign="top" class="w3-padding-small w3-border">Moderator</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][3].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.get_date($result[$i][4]).' at '.$result[$i][5].'</td>
							<td valign="top" class="w3-padding-small w3-border"><a class="w3-text-blue w3-cursor w3-decoration-null w3-bold" onclick="view_result13(\'Moderator\',\''.$result[$i][0].'\')"><i class="fa fa-envelope-open-o"></i> View</a></td>
						</tr>';
				}
			}
		}
		else if($user_type==3) //admin
		{
			...
			$stmt = $conn->prepare("select distinct(b.nr_admin_id),b.nr_admin_name,b.nr_admin_designation,(select count(c.nr_admin_id) from nr_admin_login_transaction c where c.nr_admin_id=b.nr_admin_id),(select d.nr_suadlotr_date from nr_admin_login_transaction d where d.nr_admin_id=b.nr_admin_id order by d.nr_suadlotr_date desc,d.nr_suadlotr_time desc limit 1),(select e.nr_suadlotr_time from nr_admin_login_transaction e where e.nr_admin_id=b.nr_admin_id order by e.nr_suadlotr_date desc,e.nr_suadlotr_time desc limit 1) from nr_admin_login_transaction a,nr_admin b where a.nr_admin_id=b.nr_admin_id and b.nr_admin_name like concat('%',:search_text,'%') and b.nr_admin_type='Admin' order by ".$order_by." ".$order." limit $page,5 ");
			$stmt->bindParam(':search_text', $search_text);
			$stmt->execute();
			$result = $stmt->fetchAll();
			if(count($result)!=0)
			{
				...
				for($i=0;$i<$sz;$i++)
				{
					echo '<tr>
							...
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][1].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][2].'</td>
							<td valign="top" class="w3-padding-small w3-border">Admin</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][3].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.get_date($result[$i][4]).' at '.$result[$i][5].'</td>
							<td valign="top" class="w3-padding-small w3-border"><a class="w3-text-blue w3-cursor w3-decoration-null w3-bold" onclick="view_result13(\'Admin\',\''.$result[$i][0].'\')"><i class="fa fa-envelope-open-o"></i> View</a></td>
						</tr>';
				}
			}
		}
		else if($user_type==4 && $_SESSION['admin_type']=='Super Admin') //super admin
		{
			...
			$stmt = $conn->prepare("select distinct(b.nr_admin_id),b.nr_admin_name,b.nr_admin_designation,(select count(c.nr_admin_id) from nr_admin_login_transaction c where c.nr_admin_id=b.nr_admin_id),(select d.nr_suadlotr_date from nr_admin_login_transaction d where d.nr_admin_id=b.nr_admin_id order by d.nr_suadlotr_date desc,d.nr_suadlotr_time desc limit 1),(select e.nr_suadlotr_time from nr_admin_login_transaction e where e.nr_admin_id=b.nr_admin_id order by e.nr_suadlotr_date desc,e.nr_suadlotr_time desc limit 1) from nr_admin_login_transaction a,nr_admin b where a.nr_admin_id=b.nr_admin_id and b.nr_admin_name like concat('%',:search_text,'%') and b.nr_admin_type='Super Admin' order by ".$order_by." ".$order." limit $page,5 ");
			$stmt->bindParam(':search_text', $search_text);
			$stmt->execute();
			$result = $stmt->fetchAll();
			if(count($result)!=0)
			{
				...
				for($i=0;$i<$sz;$i++)
				{
					echo '<tr>
							...
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][1].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][2].'</td>
							<td valign="top" class="w3-padding-small w3-border">Super Admin</td>
							<td valign="top" class="w3-padding-small w3-border">'.$result[$i][3].'</td>
							<td valign="top" class="w3-padding-small w3-border">'.get_date($result[$i][4]).' at '.$result[$i][5].'</td>
							<td valign="top" class="w3-padding-small w3-border"><a class="w3-text-blue w3-cursor w3-decoration-null w3-bold" onclick="view_result13(\'Super Admin\',\''.$result[$i][0].'\')"><i class="fa fa-envelope-open-o"></i> View</a></td>
						</tr>';
				}
			}
		}
