<?php
	...
	if(isset($_REQUEST['search_text']) && isset($_REQUEST['sort']) && isset($_REQUEST['search_results_from']) && isset($_REQUEST['program_id2']) && isset($_REQUEST['faculty_dept_id']) && isset($_REQUEST['faculty_id']) && $_REQUEST['faculty_dept_id']==$_SESSION['faculty_dept_id'] && $_REQUEST['faculty_id']==$_SESSION['faculty_id'])
	{
		...
		if($sort>4)
		{
			if($program_id2==-1)
			{
				$stmt = $conn->prepare("select * from nr_student where (nr_stud_id LIKE CONCAT('%',:search_text,'%') or nr_stud_name LIKE CONCAT('%',:search_text,'%')) and nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:f_d_id)  and nr_stud_status='Active'");
			}
			else
			{
				$stmt = $conn->prepare("select * from nr_student where (nr_stud_id LIKE CONCAT('%',:search_text,'%') or nr_stud_name LIKE CONCAT('%',:search_text,'%')) and nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:f_d_id) and nr_prog_id=:prog_id and nr_stud_status='Active'");
		...
		else
		{
			...
			if($program_id2==-1)
			{
				$stmt = $conn->prepare("select * from nr_student where (nr_stud_id LIKE CONCAT('%',:search_text,'%') or nr_stud_name LIKE CONCAT('%',:search_text,'%')) and nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:f_d_id)  and nr_stud_status='Active' order by ".$order_by." ".$order." limit $page,5");
			}
			else
			{
				$stmt = $conn->prepare("select * from nr_student where (nr_stud_id LIKE CONCAT('%',:search_text,'%') or nr_stud_name LIKE CONCAT('%',:search_text,'%')) and nr_prog_id in (select nr_prog_id from nr_program where nr_dept_id=:f_d_id) and nr_prog_id=:prog_id and nr_stud_status='Active' order by ".$order_by." ".$order." limit $page,5");
				$stmt->bindParam(':prog_id', $program_id2);
			}
			$stmt->bindParam(':f_d_id', $faculty_dept_id);
			$stmt->bindParam(':search_text', $search_text);
